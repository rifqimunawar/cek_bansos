...
@section('module-content')
  ...
  <div class="panel panel-inverse">
    ...
    <div
      ...
      <a href="{{ route('penandatangan.create') }}" class="btn btn-primary btn-sm">Tambah &ensp;<i
          class="fa fa-plus-square" aria-hidden="true" style="font-size: 12px"></i></a>
      <div style="display: flex; gap: 10px;">
        {{-- <a href="{{ route('penandatangan.export') }}" class="btn btn-warning btn-sm">
        ...
        {{-- <a href="{{ route('penandatangan.pdf') }}" class="btn btn-warning btn-sm">
      ...
      <table id="data-table-default" width="100%" class="table table-striped table-bordered align-middle text-nowrap">
        <thead>
          <tr>
            ...
            <th class="text-nowrap">Tanda Tangan</th>
            <th class="text-nowrap">Tindakan</th>
          </tr>
        </thead>
        <tbody>
          @foreach ($data as $item)
            <tr class="odd gradeX">
              ...
              <td>{{ $item->nama }}</td>
              <td>{{ $item->jabatan }}</td>
              <td class="text-center"><img src="{{ $item->sign }}" alt="{{ $item->nama }}"
                  style="height: 4rem; width: 8rem; object-fit: cover;"></td>
              <td>
                <a href="{{ route('penandatangan.edit', $item->id) }}">
                  <i class="fa fa-pencil-square" style="font-size: 14px; margin-right:5px"></i>
                </a>
                <a href="{{ route('penandatangan.destroy', $item->id) }}" data-confirm-delete="true">
                  <i class="fa fa-trash mx-2" style="font-size: 14px"></i>
                </a>
                {{-- <a href="{{ route('penandatangan.view', $item->id) }}">
