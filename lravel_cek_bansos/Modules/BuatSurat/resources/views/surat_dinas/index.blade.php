...
@section('module-content')
  ...
  <div class="panel panel-inverse">
    ...
    <div
      ...
      <a href="{{ route('s_masuk.create') }}" class="btn btn-primary btn-sm">Tambah &ensp;<i class="fa fa-plus-square"
          aria-hidden="true" style="font-size: 12px"></i></a>
      <div style="display: flex; gap: 10px;">
        {{-- <a href="{{ route('s_masuk.export') }}" class="btn btn-warning btn-sm">
        ...
        {{-- <a href="{{ route('s_masuk.pdf') }}" class="btn btn-warning btn-sm">
        ...
        <tbody>
          ...
          @foreach ($data as $item)
            <tr class="odd gradeX">
              ...
              <td>{{ Fungsi::format_tgl($item->tanggal_diterima) }}</td>
              <td>{{ $item->pengirim }}</td>
              <td>{{ $item->perihal }}</td>
              <td class="d-flex justify-content-center">
                <a href="{{ route('s_masuk.edit', $item->id) }}">
                  <i class="fa fa-pencil-square" style="font-size: 14px; margin-right:5px"></i>
                </a>
                <a href="{{ route('s_masuk.destroy', $item->id) }}" data-confirm-delete="true">
  ...
  <script>
    $(document).on('click', '.btn-modal-lampiran', function() {
      ...
      var url = base_url + 'simpan/masuk/' + id + '/view';
