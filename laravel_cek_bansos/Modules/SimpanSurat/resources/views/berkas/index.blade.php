...
@section('module-content')
  ...
  <div class="panel panel-inverse">
    ...
    <div
      ...
      <a href="{{ route('berkas.create') }}" class="btn btn-primary btn-sm">Tambah &ensp;<i class="fa fa-plus-square"
          aria-hidden="true" style="font-size: 12px"></i></a>
      <div style="display: flex; gap: 10px;">
        {{-- <a href="{{ route('berkas.export') }}" class="btn btn-warning btn-sm">
        ...
        {{-- <a href="{{ route('berkas.pdf') }}" class="btn btn-warning btn-sm">
      ...
      <table id="data-table-default" width="100%" class="table table-striped table-bordered align-middle text-nowrap">
        <thead>
          <tr>
            ...
            <th class="text-nowrap">Kode Berkas</th>
        ...
        <tbody>
          ...
          @foreach ($data as $item)
            <tr class="odd gradeX">
              ...
              <td>{{ $item->kode_berkas }}</td>
              <td>{{ Fungsi::format_tgl($item->tanggal_upload) }}</td>
              <td>{{ $item->deskripsi }}</td>
              <td class="d-flex justify-content-center">
                <a href="{{ route('berkas.edit', $item->id) }}">
                  <i class="fa fa-pencil-square" style="font-size: 14px; margin-right:5px"></i>
                </a>
                <a href="{{ route('berkas.destroy', $item->id) }}" data-confirm-delete="true">
  ...
  <script>
    $(document).on('click', '.btn-modal-lampiran', function() {
      ...
      var url = base_url + 'simpan/berkas/' + id + '/view';
